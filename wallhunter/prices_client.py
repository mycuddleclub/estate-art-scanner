...
def record_mutualart(artist: str, items: list) -> int:
    """Persist MutualArt comps (tier A) permanently. Returns rows added."""
    try:
        conn = _connect()
        ak = _key(artist)
        ...
        for it in items or []:
            ...
            realized = _parse_price(it.get("realized_price"))
            outcome = "sold" if realized else (
                "unsold" if it.get("estimate") else "listed")
            key = "mutualart:" + hashlib.sha1(
                f"{ak}|{title}|{realized}|{it.get('date','')}".encode()
            ).hexdigest()[:20]
            cur = conn.execute(
                ...
                (key, ak, artist.strip(), title,
                 _work_class(f"{title} {it.get('medium', '')}"), realized,
                 outcome, (it.get("estimate") or "")[:80], "",
